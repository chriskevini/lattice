...
class TestUpsertEntity:
    """Tests for upsert_entity function."""

    @pytest.mark.asyncio
    async def test_upsert_entity_creates_new(self) -> None:
        """Test creating a new entity when it doesn't exist."""
        entity_id = uuid4()

        mock_pool, mock_conn = create_mock_pool_with_transaction()
        # First query returns None (entity doesn't exist)
        # Second query returns the new ID
        mock_conn.fetchrow = AsyncMock(side_effect=[None, {"id": entity_id}])

        with patch("lattice.memory.episodic.db_pool") as mock_db_pool:
            mock_db_pool.pool = mock_pool

            result = await upsert_entity(name="Python", entity_type="language")

            assert result == entity_id
            assert mock_conn.fetchrow.call_count == 2

    @pytest.mark.asyncio
    async def test_upsert_entity_finds_existing(self) -> None:
        """Test finding an existing entity instead of creating."""
        entity_id = uuid4()

        mock_pool, mock_conn = create_mock_pool_with_transaction()
        # First query returns existing entity
        mock_conn.fetchrow = AsyncMock(return_value={"id": entity_id})
        ...
        with patch("lattice.memory.episodic.db_pool") as mock_db_pool:
            ...
            assert result == entity_id
            # Should only call once (found existing)
            mock_conn.fetchrow.assert_called_once()

    @pytest.mark.asyncio
    async def test_upsert_entity_case_insensitive_matching(self) -> None:
        ...
        assert result == entity_id
        call_args = mock_conn.fetchrow.call_args
        # Verify LOWER() is used in query for case-insensitive matching
        assert "LOWER(name) = $1" in call_args[0][0]

    @pytest.mark.asyncio
    async def test_upsert_entity_strips_whitespace(self) -> None:
        """Test that entity names have whitespace stripped."""
        entity_id = uuid4()
        ...
        assert result == entity_id
        call_args = mock_conn.fetchrow.call_args
        # Should search for stripped and lowercased name
        assert call_args[0][1] == "python"

    @pytest.mark.asyncio
    async def test_upsert_entity_without_type(self) -> None:
        """Test creating entity without specifying type."""
        entity_id = uuid4()
        ...
        with patch("lattice.memory.episodic.db_pool") as mock_db_pool:
            ...
            result = await upsert_entity(name="Python", entity_type=None)

            assert result == entity_id
            # Check INSERT query has NULL for entity_type
            insert_call = mock_conn.fetchrow.call_args_list[1]
            assert insert_call[0][2] is None

    @pytest.mark.asyncio
    async def test_upsert_entity_with_provided_connection(self) -> None:
        """Test using a provided connection instead of acquiring new one."""
        entity_id = uuid4()

        mock_conn = AsyncMock()
        mock_conn.fetchrow = AsyncMock(return_value={"id": entity_id})

        # Should NOT access db_pool when conn is provided
        result = await upsert_entity(
            name="Python", entity_type="language", conn=mock_conn
        )
    ...
    @pytest.mark.asyncio
    async def test_upsert_entity_insert_failure_raises(self) -> None:
        """Test that insert failures raise exceptions."""
        mock_pool, mock_conn = create_mock_pool_with_transaction()
        mock_conn.fetchrow = AsyncMock(side_effect=[None, Exception("Insert failed")])

        with patch("lattice.memory.episodic.db_pool") as mock_db_pool:
            mock_db_pool.pool = mock_pool

            with pytest.raises(Exception, match="Insert failed"):
                await upsert_entity(name="Python", entity_type="language")

    @pytest.mark.asyncio
    async def test_upsert_entity_insert_returns_none_raises_valueerror(self) -> None:
        """Test that INSERT returning None raises ValueError (defensive check)."""
        mock_pool, mock_conn = create_mock_pool_with_transaction()
        # First query finds nothing, second INSERT returns None (should never happen in practice)
        mock_conn.fetchrow = AsyncMock(side_effect=[None, None])

        with patch("lattice.memory.episodic.db_pool") as mock_db_pool:
            mock_db_pool.pool = mock_pool

            with pytest.raises(ValueError, match="Failed to create entity: TestEntity"):
                await upsert_entity(name="TestEntity", entity_type="test")
