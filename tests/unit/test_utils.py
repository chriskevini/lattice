...
class TestCanonicalFetching:
    """Tests for canonical entity and predicate fetching functions."""
    ...
    @pytest.mark.asyncio
    async def test_get_canonical_entities_fetches_from_database(self) -> None:
        """Test get_canonical_entities fetches from database when cache is empty."""
        mock_rows = [
            {"name": "Mother"},
            {"name": "boyfriend"},
            {"name": "marathon"},
        ...
        with patch("lattice.utils.database.db_pool") as mock_db_pool:
            ...
            result = await get_canonical_entities()
    ...
    @pytest.mark.asyncio
    async def test_get_canonical_entities_returns_empty_list(self) -> None:
        """Test get_canonical_entities returns empty list when table is empty."""
        mock_conn = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_get_canonical_entities_uses_cache(self) -> None:
        """Test get_canonical_entities returns cached result within TTL."""
        mock_rows = [{"name": "cached_entity"}]
        mock_conn = AsyncMock()
        mock_conn.fetch = AsyncMock(return_value=mock_rows)
        mock_pool = MagicMock()
        mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
        ...
        with patch("lattice.utils.database.db_pool") as mock_db_pool:
            ...
            result1 = await get_canonical_entities()
            result2 = await get_canonical_entities()

            assert result1 == result2
            assert result1 == ["cached_entity"]
            assert mock_conn.fetch.call_count == 1

    @pytest.mark.asyncio
    async def test_get_canonical_entities_cache_expires(self) -> None:
        """Test get_canonical_entities refetches after cache TTL expires."""
        import lattice.utils.database as db_module

        mock_rows1 = [{"name": "entity1"}]
        mock_rows2 = [{"name": "entity2"}]

        mock_conn = AsyncMock()
        mock_conn.fetch = AsyncMock(side_effect=[mock_rows1, mock_rows2])
        mock_pool = MagicMock()
        mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
        ...
        with patch("lattice.utils.database.db_pool") as mock_db_pool:
            ...
            result1 = await get_canonical_entities()
            assert result1 == ["entity1"]

            db_module._entities_cache_timestamp = 0

            result2 = await get_canonical_entities()
            assert result2 == ["entity2"]
            assert mock_conn.fetch.call_count == 2

    @pytest.mark.asyncio
    async def test_get_canonical_predicates_fetches_from_database(self) -> None:
        """Test get_canonical_predicates fetches from database when cache is empty."""
        mock_rows = [
            {"name": "has goal"},
            {"name": "due by"},
            {"name": "did activity"},
        ...
        with patch("lattice.utils.database.db_pool") as mock_db_pool:
            ...
            result = await get_canonical_predicates()
    ...
    @pytest.mark.asyncio
    async def test_get_canonical_predicates_returns_empty_list(self) -> None:
        """Test get_canonical_predicates returns empty list when table is empty."""
        mock_conn = AsyncMock()
        mock_conn.fetch = AsyncMock(return_value=[])
        mock_pool = MagicMock()
        mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
    ...
    @pytest.mark.asyncio
    async def test_get_canonical_predicates_uses_cache(self) -> None:
        """Test get_canonical_predicates returns cached result within TTL."""
        mock_rows = [{"name": "cached_predicate"}]
        mock_conn = AsyncMock()
        mock_conn.fetch = AsyncMock(return_value=mock_rows)
        mock_pool = MagicMock()
        mock_pool.acquire.return_value.__aenter__.return_value = mock_conn
        ...
        with patch("lattice.utils.database.db_pool") as mock_db_pool:
            ...
            result1 = await get_canonical_predicates()
            result2 = await get_canonical_predicates()

            assert result1 == result2
            assert result1 == ["cached_predicate"]
            assert mock_conn.fetch.call_count == 1

    def test_clear_canonical_cache_clears_both_caches(self) -> None:
        """Test clear_canonical_cache clears both entities and predicates caches."""
        import lattice.utils.database as db_module

        db_module._entities_cache = ["entity1", "entity2"]
        db_module._entities_cache_timestamp = 999999999
        db_module._predicates_cache = ["predicate1", "predicate2"]
        db_module._predicates_cache_timestamp = 999999999

        clear_canonical_cache()

        assert db_module._entities_cache is None
        assert db_module._predicates_cache is None

    def test_canonical_cache_ttl_is_5_minutes(self) -> None:
        """Test that CANONICAL_CACHE_TTL_SECONDS is set to 300 (5 minutes)."""
        assert CANONICAL_CACHE_TTL_SECONDS == 300
