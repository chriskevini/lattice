...
-- ----------------------------------------------------------------------------
-- canonical_entities: Canonical entity registry for normalization
-- ----------------------------------------------------------------------------
-- Stores canonical forms of entities with their variants for normalization.
-- Enables consistent entity matching across different surface forms.
-- Example: "mom" → "Mother", "bf" → "boyfriend"
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS canonical_entities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    canonical_form TEXT UNIQUE NOT NULL,
    variants TEXT[] NOT NULL DEFAULT '{}',
    category TEXT,
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_canonical_entities_canonical ON canonical_entities(canonical_form);
CREATE INDEX IF NOT EXISTS idx_canonical_entities_variants ON canonical_entities USING gin(variants);

-- ----------------------------------------------------------------------------
-- canonical_predicates: Canonical predicate registry for normalization
-- ----------------------------------------------------------------------------
-- Stores canonical forms of predicates with their variants for normalization.
-- Enables consistent predicate matching across different surface forms.
-- Example: "lives in" → "lives_in", "works at" → "works_as"
-- ----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_canonical_predicates_canonical ON canonical_predicates(canonical_form);
CREATE INDEX IF NOT EXISTS idx_canonical_predicates_variants ON canonical_predicates USING gin(variants);
